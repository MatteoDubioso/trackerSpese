    ...
    return (
      <Link
        ...
        className={`flex flex-col items-center justify-center w-full py-1 transition-all ${
          attivo ? 'text-emerald-400 scale-105' : 'text-slate-500 hover:text-white'
        }`}
      >
        <span className="text-xl">{icon}</span>
        <span className="text-[10px] font-bold uppercase mt-0.5 tracking-wide">{label}</span>
      </Link>
    );
  }
  ...
  return (
    <Link
      ...
      className={`relative px-4 py-2 rounded-xl font-semibold transition-all ${
        attivo ? 'text-white' : 'text-slate-400 hover:text-white'
      }`}
    >
      {label}
      <span
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center">
        <div className="relative">
          ...
          <div className="absolute inset-0 animate-spin rounded-full border-t-4 border-emerald-500"></div>
        </div>
      </div>
    );
  }
        ...
        <Routes>
          ...
          {!utente ? (
            <>
              ...
              {/* Se provi ad andare altrove da sloggato, torni alla Home */}
              <Route path="*" element={<Navigate to="/" />} />
            </>
          ) : (
            /* --- ROTTE PER UTENTI LOGGATI --- */
            <>
              {/* Se un loggato va sulla radice o su login, lo mandiamo in Dashboard */}
                ...
                element={
                  <>
                    ...
                    <main className="flex-1 w-full overflow-x-hidden">
                      <div className="w-full px-4 sm:px-6 md:px-8 lg:px-12 xl:px-20 py-6 max-w-7xl mx-auto">
                        <Routes>
                          ...
                          {/* Fallback per rotte inesistenti da loggato */}
                          <Route path="*" element={<Navigate to="/dashboard" />} />
                        </Routes>
                      </div>
                    </main>
                    ...
                    {/* NAVBAR MOBILE */}
                    <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-slate-900/90 backdrop-blur-xl border-t border-slate-800 h-16 flex justify-around items-center z-50 px-2">
                      <NavLink to="/dashboard" icon="🏠" label="Home" mobile />
